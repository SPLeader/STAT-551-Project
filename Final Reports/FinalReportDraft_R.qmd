...
To help determine which of the 100+ predictors to use, we used a linear regression with a LASSO penalty. Out of the 117 total variables, we use the top 8 according to the LASSO model. Most of the coefficients were absolute zero, meaning many of the variables are considerably less informative than others. By looking at the magnitudes of the top coefficients, we can compare which ones give the most information for determining the walkability index. Among the top variables are variables associated with employment entropy, road network density, and occupied housing, and we will postpone interpretation of these variables until later in the report.
...
For the Walkability dataset, we used various regeression models to make predictions on the numeric walkability index. For all models, 10-fold cross-validation was used to get a better sense of how these models would perform when predicting on unseen data. 
...
Our linear model achieved a notable R-squared value, meaning we can use it to make some pretty valuable interpretations regarding the data. There are three particularly large coefficients, which tell us that these variables have a relevance with predicting walkability. The top variables here are in agreement with our LASSO model.

According to our coefficients, a single unit increase in road network density, employment entropy, and household entropy are each associated with roughly a single unit increase in the walkability index. On the other hand, the network density has a negative correlation of about half the strength. The variable D3AAO specifically refers to the density of auto-oriented links (as opposed to pedestrian oriented links). The D3AMM variable is similar but includes both auto and pedestrian-oriented network links. That they both have a negative coefficient is not surprising; areas that are crowded with vehicles tend to be less walkable.

The RMSE of 0.982 makes our linear regression model less effective than the others, but it was definitely good enough (and interpretable enough) for us to gain some valuable insight.
