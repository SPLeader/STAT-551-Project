...
To help determine which of the 117 predictors to use, we used linear regression with a LASSO penalty term. Most of the coefficients shrunk to zero, meaning many of the variables are considerably less informative than others. By looking at the magnitudes of the top six coefficients, we can compare which variables give the most information for determining the walkability index. Among the top variables are variables associated with employment entropy, road network density, and occupied housing. We discuss specific interpretations of these variables in the Linear Regression section.

```{r, fig.cap = "Figure 9: Bar plot of top six variables, as selected by LASSO", include = TRUE}
walk_lasso_coef %>% 
...
There are three particularly large coefficients, corresponding to D2B_E8MIXA, D3A, and D2A_EPHHM. It is worth noting that the top variables here are in agreement with our LASSO variable importance.

Because these three coefficients are all close to one, this means that a single unit increase in road network density, employment entropy, and household entropy are each roughly  associated with a single unit increase in the walkability index. Conversely, the network density has a negative coefficient of about -0.5. The variable D3AAO specifically refers to the density of automobile-oriented links (as opposed to pedestrian oriented links). The D3AMM variable is similar but includes both auto and pedestrian-oriented network links. That both D3AMM and D3AAO both have a negative coefficient is not surprising; areas that are crowded with vehicles tend to be less walkable.


### Decision Tree Regression

A decision tree regressor, much like our linear regression model, has the benefit of being highly interpretable, even if it's not the most powerful model. We can create the model and interpret the top of the resulting tree similar to how we did when performing feature selection. The tree was given a very small maximum depth, so that we can avoid overfitting as well as examine the whole tree rather easily. The final decision tree achieved the worst metrics, with a higher error score and lower R2-score than any of the other final models.

At the top of our final tree is the variable D3B. This shows that having a lower density of street intersections (not including auto-oriented intersections) is an indicator of a lower walkability score. Areas with fewer intersections would be more disconnected, putting more distance between people and their destinations, so this split makes sense. The next split considers the mix of employment types, and shows that areas with lower job diversity may have a lower walkability score. On the other end of the tree is a split for D2C_TRPMX, which is a variable referring to the mix of employment as well as residential type. It suggests that having a lower diversity of both jobs and residential type also correlates with a lower walkability score.

![](../Walkability%20Outputs/walkability_decision_tree.png)

Figure #: Decision tree showing the uppermost splits (most important variables)

### Random Forest Regression

With such a large dataset, a random forest algorithm was far from the fastest option. Regardless, it has the potential to improve upon predicting power of the other models, especially in regards to overfitting. Performing grid-searches was time consuming, but we were still able to create a model with great cross-validated metrics. Our final random forest regression model had the highest R2 and lowest error of all of our models. Unfortunately, it is also the least interpretable, meaning that despite it's predicting power, it's difficult to get a lot of analyses based on the results.

### K-Neighbors Regression

Though usually performed in classification problems, KNN was a powerful regression tool in our analysis. Fit with the top 6 predictors according to our feature selection (See Table 2), the final model achieved RMSE .868 and R2 of .867. These metrics don't beat out our best random forest model, but at the very least they demonstrate that there is more than one way to tackle this regression problem. Unfortunately, the KNN model turns out to be as uninterpretable as the random forest.

To find the best parameters for KNN, we performed another grid search. The most impactful hyperparameter that we tuned was the number of neighbors, **k**, settling on a k-value of around 10 as that is where our R2-score plateaued. In general, we wouldn't want too high of a k value, lest we skyrocket the models bias. In this case, we were able to explain a high amount of the variability in the response with a pretty low value for k.
