...
dt = pickle.load(open('NWI Outputs/decision_tree_feature_selection.sav', 'rb'))
...
plt.clf()
sns.barplot(x=df_nwi_lasso_features["Feature"][:6], y=np.abs(df_nwi_lasso_features["Coefficients"][:6]))
...
```{python, fig.cap = 'Figure #: Tree plot for feature selection'}
all_features = list(nwi.drop(columns=['D1C8_OFF', 'CSA', 'OBJECTID', 'GEOID10', 'GEOID20', 'STATEFP', 'COUNTYFP', 'CSA_Name', 'CBSA_Name', 'cat', 'CBG', 'D4A', 'D4D', 'CBG', 'NatWalkInd', 'D2A_Ranked', 'D2B_Ranked', 'D3B_Ranked', 'D4A_Ranked']).columns)

plt.clf()
plot_tree(dt, proportion=True, max_depth=1, feature_names=all_features, 
        class_names=list(nwi['NatWalkInd'].unique()), filled=True)
...
```{python, fig.cap = "Talbe #: NWI Walkability Model Metrics", include = TRUE}
df_walkability_model_metrics = pd.DataFrame(
  data = {
    ...
    'MSE' : ['1.3600', '0.5960', '0.5817', '0.5794'],
    'R-Squared' : ['0.7503', '0.8895', '0.8934', '0.8929']
})

Markdown(tabulate(df_walkability_model_metrics, headers = ['Model Specification','MSE', 'R-Squared'], showindex = "never"))
```
...
![](../NWI%20Outputs/KNN_n_scores.png)
