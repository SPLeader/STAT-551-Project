...
nwi <- read_csv(here("cleaned_nwi.csv"))
```
...
data.frame(`Walkability Index` = c("1 – 5.75", "5.76 – 10.5", 
                               "10.51 – 15.25", "15.26 – 20"),
           `Category` = 
             c("Least walkable",
...
```{r, fig.cap = "Table 3. Variable summary for Walkability dataset", include = TRUE}
my_df <- data.frame(`Variable Name` = c("D2a_EpHHm", "D2b_E5MixA", "D3apo", 
                               "D3b", "D3bmm4","D3a"),
           `Variable Description` = 
             c("The mix of employment types and occupied housing",
               "5-tier employment mix (retail, office, industrial, service, and entertainment jobs",
               "Network density in terms of pedestrian-oriented paths per square mile",
...
```{r, fig.cap = "Figure 4. Overall distribution of Walkability Index", include = TRUE, message = FALSE}
...
The histogram of Walkability Index appears relatively bell-shaped and symmetric. The median Walkability Index is 13.83 and the interquartile range is 3.33. The range for the Walkability Index is 1 to 20.
...
```{r, fig.cap = "Figure 5. Distribution of Walkability Index by Population", include = TRUE}
...
```{r, fig.cap = "Figure 6. Employment Mix vs. Walkability Index", include = TRUE}
ggplot(nwi, 
       aes(x = D2B_E8MIXA, 
   ...
   theme_bw() + 
    scale_color_manual(values = c("gold", "darkred", "chocolate", "limegreen", "blue"))
```
...
```{r, fig.cap = "Table 6. Summary of Model Fits by Metric", include = TRUE}
data.frame(`Model Specification` = c("Linear Regression", "K-Neighbors Regression",
...
Our goal was to leverage machine learning to assist the EPA with their mission of furthering environmental protection, advancing environmental justice, and safeguarding communities. We specifically created tools to help the EPA improve air quality by drawing inferences from models that predict the AQI and Walkability Index of a given place. This can help the EPA inform the public of danger and lobby for projects to improve places with low walkability to reduce car dependency.

For AQI, there are two models to highlight. The first is the one with the best predictive performance, a random forest. It is able to predict a county's next-day AQI without using ozone measurements with an average error of 21 AQI points. Given that AQI is on a scale of 0-500, our model is able to give the EPA and the public a decent idea of the next day's Air Quality Index. The other model to draw attention to is the linear regression model which has less predictive power but does provide insight on which measurements matter most when predicting the next day's AQI. From this model, we can conclude that Nitrogen Dioxide levels contribute the most to predicting AQI, followed by the maximum temperature and minimum humidity levels. The EPA can use this knowledge to target their data collection to reduce bias in the data. In places where they currently don't have data, the EPA can begin by only measuring these three characteristics which lessens the burden of setting up complex weather stations. AQI prediction could be used to target communities experiencing hazardous air quality in order to distribute resources such as masks that filter harmful particles.

In terms of predicting Walkability Index, the highest-performing model was also a random forest. It was able to explain 89.34% of the variability in Walkability Index. Unfortunately, however, this model does not allow for much meaningful interpretation. The linear regression model does provide insights on characteristics of a place that matter most in determining its walkability. It was also fairly effective at making predictions with an average error of only 1.36 Walkability Index points. The information gained from this model is that street intersection density matters most, places that have a greater mix of employment opportunities and occupied housing units tend to be more walkable, and places that are larger in area are typically less walkable. Actions that could be taken based off of this are to focus on infrastructure projects that add dedicated infrastructure for pedestrians and cyclists, build denser neighborhoods with shorter block lengths and a mix of housing units alongside businesses and shops, and to ensure that if a metro area is quite large, that it is well-connected by public transit.
