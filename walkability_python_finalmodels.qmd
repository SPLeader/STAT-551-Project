...
dt.fit(X_train[dt_best_features[:5]], y_train)
dt.score(X_test[dt_best_features[:5]], y_test)
```
...
```{python}
cross_val_score(DecisionTreeRegressor(max_depth=9), X[dt_best_features[:6]], y, cv=10).mean()
```
...
cross_val_score(lr_pipeline,X, y, cv=10).mean()
...
sns.barplot(x=features, y=np.abs(coefficients))
