...
plt.savefig('NWI Outputs/NWI_histogram.png')
plt.show()
```
...
```{python}
X = nwi.drop(columns=['D1C8_OFF', 'CSA', 'OBJECTID', 'GEOID10', 'GEOID20', 'STATEFP', 'COUNTYFP', 'CSA_Name', 'CBSA_Name', 'cat', 'CBG', 'D4A', 'D4D', 'CBG', 'D2A_Ranked', 'D2B_Ranked', 'D3B_Ranked', 'D4A_Ranked', 'State', 'NatWalkInd'])
y = nwi['NatWalkInd']
...
df_nwi_lasso_features = pd.DataFrame(data = {
  'features': features,
  'coefficients': coefficients
})

# Save outputs as csv to recreate plot in final report
df_nwi_lasso_features.to_csv("NWI Outputs/NWI_lasso_features_py.csv")
...
plot_tree(dt, proportion=True, feature_names=all_features, 
        class_names=list(y.unique()), filled=True)
plt.title('Decision Tree on all predictors')
plt.savefig('NWI Outputs/decision_tree_feature_selection.png')
plt.show()
```
...
```{python}
data = nwi[['cat', 'D3B', 'D2B_E8MIXA', 'D2A_EPHHM']]
```
