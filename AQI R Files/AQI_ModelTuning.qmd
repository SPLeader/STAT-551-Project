---
title: "AQI Prediction"
author: "Sean Leader // William Medwid // Vanessa Veto // Jose Mierzejewski"
format: 
  html:
    code-fold: true
    code-line-numbers: true
    code-tools: true
    self-contained: true
editor: visual
execute:
  message: false
---

```{r packages, message = FALSE}
# Load Packages

library(tidyverse)
library(tidymodels)
library(glmnet)
library(here)
library(rpart)
library(rpart.plot)
```


# Data Preparation

```{r}
aqi_ozone <- read_csv(here("Project Data", "Combined", "aqi_ozone_all_years_prediction.csv"))

aqi_ozone <- aqi_ozone %>% 
  select(-Date, -State, -County, -AQI_cat)
```

```{r}
# Creating k fold cv's
set.seed(13)

aqi_ozone_cvs <- aqi_ozone %>% 
  vfold_cv(k = 5)
```

```{r}
# Training and validation sets for single trained models (not cross-val)
set.seed(13)

sample <- sample(c(TRUE, FALSE), nrow(aqi_ozone), replace=TRUE, prob=c(0.9,0.1))
aqi_train  <- aqi_ozone[sample, ]
aqi_test   <- aqi_ozone[!sample, ]
```

```{r}
model_scores <- data.frame(matrix(, ncol = 3, nrow = 0))
names(model_scores <- c("Model", "R-Squared", "Root Mean Squared Error"))
```

# Linear models

### Tuning basic linear model

```{r}
# Recipes
no_ozone_recipe_normalized <- recipe(AQI_next_day ~ ., data = aqi_ozone) %>% 
  step_rm(c("AQI", "max_ozone", "mean_ozone", "min_ozone")) %>% 
  step_normalize(all_predictors())

no_ozone_recipe <- recipe(AQI_next_day ~ ., data = aqi_ozone) %>% 
  step_rm(c("AQI", "max_ozone", "mean_ozone", "min_ozone"))

no_ozone_recipe_poly_normalized <- recipe(AQI_next_day ~ ., data = aqi_ozone) %>% 
  step_rm(c("AQI", "max_ozone", "mean_ozone", "min_ozone")) %>% 
  step_poly(all_predictors(), degree = 2) %>% 
  step_normalize(all_predictors())

full_recipe <- recipe(AQI_next_day ~ ., data = aqi_ozone) %>% 
  step_normalize(all_predictors())
```


```{r}
#Tuning different mixture models
lin_reg_tune <- linear_reg(penalty = tune(), mixture = tune()) %>%
  set_engine("glmnet") %>%
  set_mode("regression")

no_ozone_recipe_normalized <- recipe(AQI_next_day ~ ., data = aqi_ozone) %>% 
  step_rm(c("AQI", "max_ozone", "mean_ozone", "min_ozone")) %>% 
  step_normalize(all_predictors())

lam_grid = grid_regular(penalty(), mixture(c(0,1)), levels = 3)

lm_wf <- workflow() %>% 
  add_model(lin_reg_tune) %>% 
  add_recipe(no_ozone_recipe_normalized)

grid_results <- lm_wf %>% 
  tune_grid(resamples = aqi_ozone_cvs, grid = lam_grid)


model_title = "Linear Model"
model_rsq = grid_results %>% 
  collect_metrics() %>% 
  filter(.metric == "rsq") %>% pull(mean) %>% max()
model_rmse = grid_results %>% 
  collect_metrics() %>% 
  filter(.metric == "rmse") %>% pull(mean) %>% max()

# Not included because polynomial is better
# model_scores <- model_scores %>% rbind(c(model_title, model_rsq, model_rmse))

grid_results %>% 
  collect_metrics() %>% 
  filter(.metric == "rsq") %>% 
  arrange(desc(mean))

grid_results %>% 
  collect_metrics() %>% 
  filter(.metric == "rmse") %>% 
  arrange(mean)
```

Out of many penalty and mixture parameters, we found a mixture of 1/2 to work best.

### Tuning polynomial linear model

```{r}
poly_reg_tune <- linear_reg(penalty = tune(), mixture = tune()) %>%
  set_engine("glmnet") %>%
  set_mode("regression")

no_ozone_recipe_poly_normalized <- recipe(AQI_next_day ~ ., data = aqi_ozone) %>% 
  step_rm(c("AQI", "max_ozone", "mean_ozone", "min_ozone")) %>% 
  step_poly(all_predictors(), degree = 2) %>% 
  step_normalize(all_predictors())

lam_grid = grid_regular(penalty(), mixture(c(0,1)), levels = 3)

lm_wf <- workflow() %>% 
  add_model(lin_reg_tune) %>% 
  add_recipe(no_ozone_recipe_poly_normalized)

grid_results <- lm_wf %>% 
  tune_grid(resamples = aqi_ozone_cvs, grid = lam_grid)

# Save metrics

model_title = "Linear Model"
model_rsq = grid_results %>% 
  collect_metrics() %>% 
  filter(.metric == "rsq") %>% pull(mean) %>% max()
model_rmse = grid_results %>% 
  collect_metrics() %>% 
  filter(.metric == "rmse") %>% pull(mean) %>% max()

model_scores <- model_scores %>% rbind(c(model_title, model_rsq, model_rmse))

# Display results

grid_results %>% 
  collect_metrics() %>% 
  filter(.metric == "rsq") %>% 
  arrange(desc(mean))

grid_results %>% 
  collect_metrics() %>% 
  filter(.metric == "rmse") %>% 
  arrange(mean)
```

### Single linear model residuals

```{r}
#Using same recipe as above
lm_best_spec <- linear_reg(penalty = 0, mixture = 1) %>%
  set_engine("glmnet") %>%
  set_mode("regression")

no_ozone_recipe_poly_normalized <- recipe(AQI_next_day ~ ., data = aqi_ozone) %>% 
  step_rm(c("AQI", "max_ozone", "mean_ozone", "min_ozone")) %>% 
  step_poly(all_predictors(), degree = 2) %>% 
  step_normalize(all_predictors())

lm_best_wf <- workflow() %>% 
  add_model(lm_best_spec) %>% 
  add_recipe(no_ozone_recipe_poly_normalized)

lm_best_fit <- lm_best_wf %>% 
  fit(aqi_train)

lm_preds <- lm_best_fit %>% 
  predict(aqi_test) %>% 
  mutate(truth = aqi_test$AQI_next_day,
         err = .pred - truth)

lm_best_fit %>% 
  extract_fit_parsnip() %>% 
  tidy() %>% 
  arrange(desc(abs(estimate)))
```

```{r}
lm_preds %>% 
  ggplot(aes(x = .pred, y = err)) +
  geom_point() +
  geom_smooth()
```

When our linear model predicts low o-zone values, it is consistently biased towards under-prediction.

### Interpretable linear model coef

```{r}
lm_best_spec <- linear_reg(penalty = 3.5, mixture = 1) %>%
  set_engine("glmnet") %>%
  set_mode("regression")

#Using same recipe as above

lm_best_wf <- workflow() %>% 
  add_model(lm_best_spec) %>% 
  add_recipe(no_ozone_recipe_normalized)

lm_best_fit_interpretable <- lm_best_wf %>% 
  fit(aqi_train)

lm_preds_interpretable <- lm_best_fit_interpretable %>% 
  predict(aqi_test) %>% 
  mutate(truth = aqi_test$AQI_next_day,
         err = .pred - truth)

lm_best_fit_interpretable %>% 
  extract_fit_parsnip() %>% 
  tidy() %>% 
  arrange(desc(abs(estimate)))
```

### Interpretable linear model efficacy

```{r}
no_ozone_recipe_simplified_normalized <- recipe(AQI_next_day ~ min_humid + max_temp + max_no2, data = aqi_ozone) %>% 
  step_normalize(all_predictors())

lam_grid = grid_regular(penalty(), mixture(c(0,1)), levels = 3)

lm_wf <- workflow() %>% 
  add_model(lin_reg_tune) %>% 
  add_recipe(no_ozone_recipe_simplified_normalized)

grid_results <- lm_wf %>% 
  tune_grid(resamples = aqi_ozone_cvs, grid = lam_grid)

grid_results %>% 
  collect_metrics() %>% 
  filter(.metric == "rsq") %>% 
  arrange(desc(mean))

grid_results %>% 
  collect_metrics() %>% 
  filter(.metric == "rmse") %>% 
  arrange(mean)
```

The best RMSE with just 3 best variables was 26.16919, not too much worse than the best models using all variables.

# Random Forest 

### Tuning

```{r}
rf_mod <- rand_forest(mtry = tune(), min_n = tune(), trees = 1000) %>%
  set_engine("ranger") %>%
  set_mode("regression")


#basic_recipe <- recipe(AQI_next_day ~ ., data = aqi_ozone) %>% 
#  step_normalize() %>% 
#  step_rm(c("Date", "State", "County", "AQI_cat"))


rf_wf <- workflow() %>% 
  add_model(rf_mod) %>% 
  add_recipe(no_ozone_recipe)


rf_grid <- grid_regular(min_n(c(1, 2)),
                        mtry(c(4, 8)),
                        levels = 1)

rf_grid_result <- rf_wf %>% 
  tune_grid(resamples = aqi_ozone_cvs,
            grid = rf_grid)

# Save metrics

model_title = "Random Forest"
model_rsq = rf_grid_result %>% 
  collect_metrics() %>% 
  filter(.metric == "rsq") %>% pull(mean) %>% max()
model_rmse = rf_grid_result %>% 
  collect_metrics() %>% 
  filter(.metric == "rmse") %>% pull(mean) %>% max()

model_scores <- model_scores %>% rbind(c(model_title, model_rsq, model_rmse))

# Print results

rf_grid_result %>% 
  collect_metrics() %>% 
  filter(.metric == "rsq") %>% 
  arrange(desc(mean))

rf_grid_result %>% 
  collect_metrics() %>% 
  filter(.metric == "rmse") %>% 
  arrange(mean)
```

### Single Random Forest Restiduals

```{r}
rf_best_spec <- rand_forest(mtry = 6, min_n = 4, trees = 1000) %>%
  set_engine("ranger") %>%
  set_mode("regression")

#Using same recipe as aboev

rf_best_wf <- workflow() %>% 
  add_model(rf_best_spec) %>% 
  add_recipe(no_ozone_recipe)

rf_best_fit <- rf_best_wf %>% 
  fit(aqi_train)

rf_preds <- rf_best_fit %>% 
  predict(aqi_test) %>% 
  mutate(truth = aqi_test$AQI_next_day,
         err = .pred - truth)

rf_pred_plot <- rf_preds %>% 
  ggplot(aes(x = .pred, y = err)) +
  geom_point() +
  geom_smooth() +
  xlab("Random Forest Prediction") +
  ylab("Error")
```


Compared to our linear model, a random forest is much less biased. For its predictions between 0 and 100,the random forest isn't perfect, but isn't biased high or low. Above 100 predicted AQI, the random forest develops a tendency to under-predict, however.

# Decision Tree

### Tuning

```{r}
tree_mod <- decision_tree(tree_depth = tune(), cost_complexity = tune()) %>%
  set_engine("rpart") %>%
  set_mode("regression")

no_ozone_recipe <- recipe(AQI_next_day ~ ., data = aqi_ozone) %>% 
  step_rm(c("AQI", "max_ozone", "mean_ozone", "min_ozone"))

dt_wf <- workflow() %>%
  add_recipe(no_ozone_recipe) %>%
  add_model(tree_mod)

dt_grid <- grid_regular(tree_depth(),
                        cost_complexity(),
                        levels = 3
                        )

dt_grid_result <- dt_wf %>% 
  tune_grid(resamples = aqi_ozone_cvs,
            grid = dt_grid)

# Save metrics

model_title = "Decision Tree"
model_rsq = dt_grid_result %>% 
  collect_metrics() %>% 
  filter(.metric == "rsq") %>% pull(mean) %>% max()
model_rmse = dt_grid_result %>% 
  collect_metrics() %>% 
  filter(.metric == "rmse") %>% pull(mean) %>% max()

model_scores <- model_scores %>% rbind(c(model_title, model_rsq, model_rmse))

# Print results

dt_grid_result %>% 
  collect_metrics() %>% 
  filter(.metric == "rsq") %>% 
  arrange(desc(mean))

dt_grid_result %>% 
  collect_metrics() %>% 
  filter(.metric == "rmse") %>% 
  arrange(mean)
```


### Single Decision Tree for residuals

```{r}
tree_mod <- decision_tree(cost_complexity = 1.000000e-10, tree_depth = 8) %>%
  set_engine("rpart") %>%
  set_mode("regression")

tree_wflow <- workflow() %>%
  add_recipe(no_ozone_recipe) %>%
  add_model(tree_mod)
```

```{r}
tree_results <- tree_wflow %>%
  fit_resamples(aqi_ozone_cvs)

tree_results %>% 
  collect_metrics() %>% 
  filter(.metric == "rsq")

tree_results %>% 
  collect_metrics() %>% 
  filter(.metric == "rmse")
```

```{r}
tree_fit_1 <- tree_wflow %>%
  fit(aqi_train)

tree_preds <- tree_fit_1 %>% 
  predict(aqi_test) %>% 
  mutate(truth = aqi_test$AQI_next_day,
         err = .pred - truth)

tree_pred_plot <- tree_preds %>% 
  ggplot(aes(x = .pred, y = err)) +
  geom_point() +
  geom_smooth() +
  xlab("Decision Tree Prediction") +
  ylab("Error")
```

### Tree plotted

```{r}
tree_mod <- decision_tree() %>%
  set_engine("rpart") %>%
  set_mode("regression")

tree_wflow <- workflow() %>%
  add_recipe(no_ozone_recipe) %>%
  add_model(tree_mod)

tree_fit_1 <- tree_wflow %>%
  fit(aqi_train)

tree_fitted <- tree_fit_1 %>% 
  pull_workflow_fit()

# Save this output as "AQI Outputs/AQI_decision_tree_r.png"
rpart.plot(tree_fitted$fit)
```

# Combined Results Plot

```{r}
all_preds <- lm_preds %>% mutate(model = "Linear\nModel") %>% 
  rbind(tree_preds %>% mutate(model = "Decision\nTree")) %>%
  rbind(rf_preds %>% mutate(model = "Random\nForest")) %>% 
  mutate(model = factor(model, levels = c("Linear\nModel", "Decision\nTree", "Random\nForest")))
```

```{r}
# Saving residuals to reproduce the below plot in final document
all_preds %>% 
  write.csv(here("AQI Outputs","AQI_df_all_preds_r.csv"), row.names = FALSE)
```


```{r}
all_preds %>% 
  ggplot(aes(x = .pred, y = err)) +
  geom_point() +
  geom_hline(yintercept = 0) + 
  geom_smooth(se = FALSE, color = "red") +
  facet_grid(rows = vars(model)) +
  xlab("Prediction") +
  ylab("Error") +
  theme_bw() +
  theme(strip.text.y.right = element_text(angle = 0))
```


```{r}
# Saving model performaces
model_scores
model_scores %>% 
  write.csv(here("AQI Outputs","AQI_model_metrics_r.csv"), row.names = FALSE)
```

