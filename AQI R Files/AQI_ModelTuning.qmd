...
no_ozone_recipe_normalized <- recipe(AQI_next_day ~ min_humid + max_temp + max_no2, data = aqi_ozone) %>% 
...
full_recipe <- recipe(AQI_next_day ~ ., data = aqi_ozone) %>% 
  step_normalize(all_predictors()) # %>%  step_interact(terms = ~max_wind:mean_ozone + max_wind : max_ozone)
