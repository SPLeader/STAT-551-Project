...
grid['max_features'] = [4, 6, 8] #np.arange(0, 1, 0.5)
...
### Single Random Forest for residuals
...
plt.savefig("AQI_decision_tree_py.png")
plt.close()
```
...
df_lm_preds.to_csv("AQI_data_output/AQI_df_lm_preds_py.csv")
df_dt_preds.to_csv("AQI_data_output/AQI_df_dt_preds_py.csv")
df_rf_preds.to_csv("AQI_data_output/AQI_df_rf_preds_py.csv")
```
...
axs[0] = sns.lmplot(x='preds',
    y='residuals',  
    data = df_lm_preds, lowess = True, ax = axs[0])

axs[1] =sns.lmplot(x='preds',
    y='residuals',  
    data = df_rf_preds, lowess = True, ax = axs[1])
    
axs[2] = sns.lmplot(x='preds',
    y='residuals',  
    data = df_dt_preds, lowess = True, ax = axs[2])
